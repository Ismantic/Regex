...
public:
    ...
    int MatchAt(const char* data, size_t len) {
        ...
        while (pos < len) {
            size_t bytes;
            uint32_t cp = DecodeUTF8At(std::string_view(data, len), pos, &bytes);
            if (bytes == 0) break;
            int next = Step(cur, cp);
            if (next < 0) break;
            cur = next;
...
public:
    ...
    bool Match(std::string_view text) { return dfa_.Match(text); }
...
bool CheckSplit(std::string_view input, std::string_view space, int cut,
                const std::vector<std::string_view>& expected) {
    regex::TokenizerRegex tokenizer(space, cut);
    const auto actual = tokenizer.Split(input);
    if (actual == expected) return true;

    std::cerr << "FAIL Split(\"" << input << "\", cut=" << cut << ")\n";
    return false;
}
...
int main(int argc, char** argv) {
    ...
    if (argc >= 2) {
        ...
        for (auto& l : lines) total_tokens += tok.Split(l).size();
    ...
    const std::string sp = "▁";
    int failures = 0;
    failures += !CheckSplit("hello▁world", sp, 0, {"hello", "▁world"});
    failures += !CheckSplit("a▁▁b", sp, 0, {"a", "▁", "▁b"});
    failures += !CheckSplit("▁,a", sp, 0, {"▁,", "a"});
    failures += !CheckSplit("hello123世界", sp, 0, {"hello", "123", "世界"});
    failures += !CheckSplit("they'll", sp, 0, {"they", "'ll"});
    failures += !CheckSplit("▁don't", sp, 1, {"▁", "don't"});
    failures += !CheckSplit("hello123你好", sp, 1, {"hello", "123", "你好"});
    if (failures != 0) return 1;

    // --- GPT-4 tokenizer ---
    std::cout << "=== GPT-4 Tokenizer ===\n\n";

    regex::TokenizerRegex tokenizer(" ");
    std::cout << "\n";
    ...
    for (auto& tc : cases) {
        auto tokens = tokenizer.Split(tc.in);
        std::cout << "'" << tc.in << "'\n  -> [";
